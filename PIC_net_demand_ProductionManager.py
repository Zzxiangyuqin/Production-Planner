...
class PIC_ProductionManager:
    ...
    def produce_pic_demand_in_month(self, date):
        ...
        for key, value in sorted_demand.items():
            ...
            if available_inventory < total_needed:
                ...
                """
                ratio_map = self.compute_ratios(init_pic_demand, sorted_demand, order_remaining)            
                for cust, prod in value:                                
                    ratio = ratio_map[key][(cust, prod)]
                    allocate_amount = int(self.pic_inventory.get(mat, 0) * ratio)
                    # 分配量处以用量，得到实际可生产的数量
                    mat_details = self.get_pic_details(cust, prod)
                    for m in mat_details:
                        if m['mat'] == mat:
                    ...
                    # 更新库存
                    if mat in self.pic_inventory:
                        self.pic_inventory[mat] -= allocate_amount
                    else:
    ...
    def compute_ratios(self, init_pic_demand, sorted_demand, order_remaining):
        ratio_map = {}
        for key, orders in sorted_demand.items():
            total = sum(init_pic_demand.get((cust, prod), 0) for cust, prod in orders)
...
if __name__ == "__main__":
    ...
    # 输出全年生产情况到.txt文件
    with open("PIC_annual_production_summary.txt", "w") as f:
    ...
    with open("PIC_annual_production_record.txt", "w") as f:
        for date, record in annual_production_record.items():
            ...
            f.write("\n")
    # 输出全年生产数量到.txt文件
    with open("PIC_annual_production_num.txt", "w") as f:
